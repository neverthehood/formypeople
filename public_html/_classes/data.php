...
class data{
    ...
    static function attributeForm($array){
        ...
        $form[]=array('name'=>'attr[folderhide]', 'label'=>'Не отображать в свойствах папки', 'type'=>'checkbox', 'class'=>'checkbox', 'value'=>$attr['folderhide']);
        $form[]=array('name'=>'attr[childhide]', 'label'=>'Не отображать в свойствах потомков', 'type'=>'checkbox', 'class'=>'checkbox', 'value'=>$attr['childhide']);
        $form[]='-';
...
static function attr(){
    ...
    static function entitySave($array){
            ...
            foreach($allMultiFields AS $v){
                foreach($opt['attr'] AS $k=>$kk){
                    if($kk['id']==$v && $kk['source']=='' && $kk['type']!='file' && $kk['type']!='child'){
	                    // Вставили пустое значение
                        if(!isset($tables[$kk['dbTable']][$k])) {
                        	$tables[$kk['dbTable']][$k]='';
                        }
                        break;
                    }
        ...
        foreach($tables AS $key=>$v){
            ...
            if(mysql::errno()!=0){
                $error.='<div class="error">Ошибка MYSQL '.mysql::errno().' : '.mysql::error().' in query '.mysql::$query.'</div>';
            }
            $newEntityId=true;
        }
...
SELECT '.$etId.', '.$etValue.' FROM `'.$etTable.'` WHERE '.$etWhere.'">'.$val['element'].'<i class="ic-db" style="font-size:12px;margin-left:6px;"></i></td>';
    ...
    static function showFormFiles($mn, $val, $array, $files){
	    ...
	    if(count($array[$val['alias']])>=1) $out.=self::showFiles($array[$val['alias']],$mainFile, $val['view'], $val);
